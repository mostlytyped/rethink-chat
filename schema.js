...
exports.resolvers = {
  Subscription: {
    chatAdded: {
      async subscribe(parent, args, context, info) {
        return new RethinkIterator(rethink.table('chats').filter({ roomId: args.room }), context.conn);
      }
    },
  },
  Mutation: {
  ...
  Query: {
    async chats(parent, args, context, info) {
      const cursor = await rethink.table('chats').filter({ roomId: args.room }).orderBy(rethink.desc('ts')).run(context.conn);
      return await cursor.toArray();
    },

    async rooms(parent, args, context, info) {
      const cursor = await rethink.table('chats')('roomId').distinct().run(context.conn);
      return await cursor.toArray();
    }
  }
}